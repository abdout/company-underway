...
const ActivityWrapper: React.FC<WrapperProps> = ({ params }) => {
  ...
  useEffect(() => {
    ...
    if (project && project._id === id && loadedProjectId.current === id) {
      console.log("ITP ActivityWrapper: Project already loaded for this ID, skipping fetch");
      return;
    }
    
    console.log("ITP ActivityWrapper: Fetching project with ID", id);
    fetchProject(id);
  ...
  useEffect(() => {
    if (project) {
      console.log("ITP ActivityWrapper: Processing project data", project);
