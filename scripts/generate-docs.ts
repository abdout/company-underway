...
import { sidebarData } from '../src/components/template/sidebar/constant';
...
const toUrlPath = (str: string | undefined): string => {
  if (!str) return '';
  // Replace slashes with hyphens or another safe character
  // This ensures that "Unbalance/Phase Loss" becomes "unbalance-phase-loss" instead of creating a subdirectory
  return str.toLowerCase().replace(/\//g, '-').replace(/\s+/g, '-');
};

// Create a basic MDX template for a specific activity
const createMdxTemplate = (item: SidebarItem, subitem: Activity, activity: string): string => {
  return `---
title: ${activity}
...
const generateActivityMdx = (item: SidebarItem, subitem: Activity, activity: string): void => {
...
const createIndexFile = (dirPath: string, title: string, items: string[]): void => {
...
const generateAllDocs = (): void => {
  ...
  sidebarData.forEach((item: SidebarItem) => {
    ...
    item.subitems.forEach((subitem: Activity) => {
